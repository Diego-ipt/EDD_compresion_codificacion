...
public:
    ...
    string descomprimir(queue<pair<string, int>> mensaje_comp) {
    ...
    int tamano_mensaje_comp(queue<pair<string, int>> mensaje_comp) {
        return mensaje_comp.size() * sizeof(pair<string, int>);
    }
    ~METODOS_LZ() = default;
};
// queue<pair<string, int>> comprimir(string& mensaje) {
